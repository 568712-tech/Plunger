...
void main()
{
    ...
    vec3 albedo = sampledColor * uBaseColor;
    ...
    color += albedo * (0.85 * sunDiffuse * sunVisibility);
    color += vec3(1.0, 0.98, 0.95) * (0.35 * sunSpecular * sunVisibility);
    ...
    for (int index = 0; index < clampedCount; ++index) {
        ...
        color += lightColor * (0.22 * specular);
    }
